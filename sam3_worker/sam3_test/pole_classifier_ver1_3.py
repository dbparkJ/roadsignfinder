...
OUTPUT_DIR = "/home/geonws/workspace/2026_project/roadsign_finder/test_file/yaw_0/TRACK03"

DEBUG = True
DEBUG_DIR = "/home/geonws/workspace/2026_project/roadsign_finder/sam3_worker/sam3_test/debug_v1_3"
...
MIN_MASK_AREA = 400  # minimum mask pixel area to accept
...
def yolo_worker(input_q, output_q):
    ...
    while True:
        ...
        image.load()
        yolo_results = yolo(image, verbose=False, conf=0.8)[0]
        boxes_xyxy = []
        if yolo_results.boxes is not None and len(yolo_results.boxes) > 0:
            boxes_xyxy = yolo_results.boxes.xyxy.cpu().numpy()
        output_q.put((image_path, image, boxes_xyxy))
        input_q.task_done()


def sam_worker(input_q, save_q):
    ...
    while True:
        ...
        if item is None:
            input_q.task_done()
            break
        image_path, image_rgb, boxes_xyxy = item
        process_image(model, processor, image_path, image_rgb, boxes_xyxy, save_q)
        input_q.task_done()
...
def process_image(model, processor, image_path, image_rgb, boxes_xyxy, save_q):
    ...
    pole_masks_full = run_sam3_text(model, processor, image_rgb, text="sign pole")
    ...
    if DEBUG and full_union is not None:
        ...
        save_q.put((f"{debug_path}/00_full_poles.png", dbg.convert("RGB")))
    ...
    for idx, box in enumerate(boxes_xyxy):
        ...
        if DEBUG:
            ...
            save_q.put((f"{debug_path}/01_vstrip_{idx}.png", v_dbg))
            h_dbg = h_strip.copy()
            d2 = ImageDraw.Draw(h_dbg)
            d2.rectangle([x1 - hx1, y1 - hy1, x2 - hx1, y2 - hy1], outline=(0, 255, 0), width=2)
            save_q.put((f"{debug_path}/02_hstrip_{idx}.png", h_dbg))

        v_masks = filter_masks_by_area(
            run_sam3_text(model, processor, v_strip, text="pole"),
            MIN_MASK_AREA,
        )
        h_masks = filter_masks_by_area(
            run_sam3_text(model, processor, h_strip, text="pole"),
            MIN_MASK_AREA,
        )
        ...
        if DEBUG and v_union is not None:
            ...
            dbg.paste(m_img, (0, 0), m_img)
            save_q.put((f"{debug_path}/03_vstrip_poles_{idx}.png", dbg.convert("RGB")))
        if DEBUG and h_union is not None:
            dbg = h_strip.convert("RGBA")
            m_img = mask_to_rgba(h_union, (0, 120, 255), alpha=110)
            dbg.paste(m_img, (0, 0), m_img)
            save_q.put((f"{debug_path}/04_hstrip_poles_{idx}.png", dbg.convert("RGB")))
        ...
        if DEBUG and h_full is not None:
            ...
            dbg.paste(m_img, (0, 0), m_img)
            save_q.put((f"{debug_path}/05_hfull_{idx}.png", dbg.convert("RGB")))
    ...
    save_q.put((output_path, overlay))
...
def main():
    ...
    save_q, save_thread = start_save_worker()

    input_list = list_images(IMAGE_DIR if os.path.isdir(IMAGE_DIR) else IMAGE_PATH)

    yolo_q = queue.Queue(maxsize=2)
    sam_q = queue.Queue(maxsize=2)

    yolo_thread = threading.Thread(target=yolo_worker, args=(yolo_q, sam_q), daemon=True)
    sam_thread = threading.Thread(target=sam_worker, args=(sam_q, save_q), daemon=True)
    yolo_thread.start()
    sam_thread.start()

    for image_path in input_list:
        yolo_q.put(image_path)
    yolo_q.put(None)
